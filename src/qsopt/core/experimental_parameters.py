...
class ExperimentalParameters:
    ...
    def _update_measurement_times(self) -> None:
        ...
        self._measurement_times = np.array(self.measurement.measurement_times)*self.physical_constants.inverse_pulse_width
    ...
    def measurement_times(self, value: np.ndarray) -> None:
        ...
        self.measurement.measurement_times = list(value / self.physical_constants.inverse_pulse_width)
        self._update_measurement_times()
    ...
    def __str__(self) -> str:
        ...
        lines.append("\nSYSTEM DIMENSIONS")
        lines.append("-" * 30)
        total_dim = self.system_dims.cavity_levels * self.system_dims.qubit_levels * self.system_dims.field_levels
        ...
        dim_valid = (self.system_dims.cavity_levels >= 2 and 
                    self.system_dims.qubit_levels >= 2 and 
                    self.system_dims.field_levels >= 2)
        ...
        lines.append(f"  Photon cavity coupling: {self.physical_constants.photon_cavity_coupling:>6.4f}")
        lines.append(f"  Inverse pulse width:  {self.physical_constants.inverse_pulse_width:>8.4f}")
        
        # Validation flags for constants
        const_valid = (self.physical_constants.chi > 0 and 
                      self.physical_constants.photon_cavity_coupling > 0 and 
                      self.physical_constants.inverse_pulse_width > 0)
        ...
        meas_valid = (n_measurements >= 2 and 
                     sorted(self.measurement.measurement_times) == self.measurement.measurement_times)
        ...
        total_noise = (self.noise_config.depolarizing + 
        ...
        if self.noise_config.custom_operators is not None:
            ...
            lines.append(f"  Custom operators:     None")
            
        # Validation flags for noise
        noise_valid = (self.noise_config.depolarizing >= 0 and 
                      self.noise_config.dephasing >= 0 and 
                      self.noise_config.relaxation >= 0)
    ...
    def __repr__(self) -> str:
        ...
        total_dim = self.system_dims.cavity_levels * self.system_dims.field_levels * self.system_dims.qubit_levels
        n_meas = len(self.measurement.measurement_times)
        chi_coupling_ratio = self.physical_constants.chi / self.physical_constants.photon_cavity_coupling
        return (f"ExperimentalParameters("
                f"dims={self.system_dims.field_levels}x{self.system_dims.cavity_levels}x{self.system_dims.qubit_levels}={total_dim}, "
