...
class QuantumCircuit:
    ...
    def update_parameters(self, parameter_values: List[float]) -> None:
        ...
        if len(parameter_values) != len(params_dict):
            raise ValueError(
                f"Expected {len(params_dict)} parameters, got {len(parameter_values)}"
            )
...
def create_layer(circuit: QuantumCircuit, gate_type: type, 
                 parameters: List[float], qubits: List[int] = None,
...
def create_entangling_layer(circuit: QuantumCircuit, gate_type: type,
...
if __name__ == "__main__":
    import numpy as np
    from .gates import RXGate, RYGate, RZGate, HadamardGate, CNOTGate
    ...
    circuit.set_trainable_parameters({
        "gate_1_theta": jnp.array(np.pi/2),
        "gate_3_theta": jnp.array(np.pi)
    })
