...
class Experiment:
    ...
    def _build_operators(self) -> None:
        ...
        if not hasattr(self.experimental_parameters, "nlev") or not hasattr(
            self.experimental_parameters, "qlev"
        ):
    ...
    def _build_hamiltonians(self) -> None:
        ...
        if not hasattr(self.experimental_parameters, "chi") or not hasattr(
            self.experimental_parameters, "gm"
        ):
        ...
        with qt.CoreOptions(default_dtype="jax"):
            ...
            H_dispersive = (
                -self.experimental_parameters.chi
                * self._operators["ac"]
                * self._operators["a"]
                * self._operators["sz"]
            )
            ...
            self._operators.update(
                {
                    ...
                    "H_total": qt.QobjEvo(
                        [H_dispersive, [H_coupling, coupling_function]], args=args
                    ),
                    "coupling_args": args,
                }
            )
    ...
    def _build_solvers(self) -> None:
        ...
        L_photon = (
            qt.QobjEvo(
                [self._operators["ain"], self._get_coupling_function()],
                args=self._operators["coupling_args"],
            )
            + jnp.sqrt(self.experimental_parameters.gm) * self._operators["a"]
        )
    ...
    def _build_custom_initial_state(self):
        ...
        # For now, return a default state (vacuum for all systems)
        return (
            qt.basis(self.experimental_parameters.nlev, 0)
            * qt.basis(self.experimental_parameters.nlev, 0)
            * qt.basis(self.experimental_parameters.qlev, 0)
        ).proj()

    def _setup_measurement_times(self) -> None:
        """Setup measurement times based on parameters."""
        if hasattr(self.experimental_parameters, "N_meas") and hasattr(
            self.experimental_parameters, "tstep"
        ):
    ...
    def run_sensing_protocol(
        self,
        rotation_angles: List[float],
        with_photon: bool = True,
        random_timing: bool = False,
        protocol_type: str = "theta_delta",
    ) -> float:
        """
    ...
    def _simulate_protocol(
        self, solver, rho, rotation_angles, tmeas, random_timing, protocol_type="theta_delta"
    ):
        """
        ...
        for kt in range(len(tmeas[:-1])):
            ...
            evolution_result = solver.run(
                rho_rotated, [t0, t1], args=self._operators.get("coupling_args", {})
            )
    ...
    def __repr__(self) -> str:
        ...
        exp_params_name = (
            type(self.experimental_parameters).__name__ if self.experimental_parameters else None
        )
        train_params_name = (
            type(self.trainable_parameters).__name__ if self.trainable_parameters else None
        )
        noise_model_name = type(self.noise_model).__name__ if self.noise_model else None
        return (
            f"Experiment("
            f"experimental_parameters={exp_params_name}, "
            f"trainable_parameters={train_params_name}, "
            f"noise_model={noise_model_name}, "
            f"status={status})"
        )
