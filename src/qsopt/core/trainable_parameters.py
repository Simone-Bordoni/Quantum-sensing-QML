...
class TrainableParameters:
    ...
    def add_rotation_angles(
        ...
        if len(names_list) != len(values):
            raise ValueError(
                f"Number of names ({len(names_list)}) must match number of values ({len(values)})"
            )
        ...
        if isinstance(trainable, bool):
            ...
            if len(trainable_list) != len(names_list):
                raise ValueError(
                    f"Number of trainable flags ({len(trainable_list)}) must match number of parameters ({len(names_list)})"
                )
        ...
        for param_name, value, is_trainable in zip(names_list, values, trainable_list):
            ...
            param = Parameter(
                idx, param_name, ParameterType.ROTATION_ANGLE, float(value), is_trainable
            )
            self.parameters.append(param)

            # Rotation angles are periodic with period 2π
            self.constraints[idx] = ParameterConstraints(periodic=True, period=2 * np.pi)
    ...
    def add_measurement_interval(
        ...
        if grid_resolution is not None and grid_resolution <= 0:
            raise ValueError(
                f"Grid resolution must be positive when provided, got {grid_resolution}"
            )

        for param_name, value, is_trainable in zip(names_list, values, trainable_list):
            idx = len(self.parameters)
            param = Parameter(
                idx, param_name, ParameterType.MEASUREMENT_TIME, float(value), is_trainable
            )
            self.parameters.append(param)

            # Measurement intervals must be strictly positive
            self.constraints[idx] = ParameterConstraints(
                min_value=min_interval, max_value=None, periodic=False  # No upper bound
            )
    ...
    def __repr__(self) -> str:
        ...
        rotation_angles = [
            p for p in self.parameters if p.param_type == ParameterType.ROTATION_ANGLE
        ]
        measurement_times = [
            p for p in self.parameters if p.param_type == ParameterType.MEASUREMENT_TIME
        ]
        ...
        if rotation_angles:
            ...
            for param in rotation_angles:
                ...
                lines.append(
                    f"    {param.name}: {param.value:.4f} rad ({angle_deg:.2f}°){trainable_str}"
                )
