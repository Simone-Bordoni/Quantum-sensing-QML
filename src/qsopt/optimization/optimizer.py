...
class Optimizer:
    ...
    def _setup_optimizer(self) -> None:
        ...
        if self.config.use_lr_schedule:
            self._lr_schedule = optax.exponential_decay(
                base_lr, self.config.decay_steps, self.config.decay_rate
            )
        else:
        ...
        optimizer_dict = {
            ...
            OptimizerType.RMSPROP: optax.rmsprop(
                self._lr_schedule, decay=0.9, eps=self.config.epsilon
            ),
    ...
    def optimize_sensing_contrast(
        ...
        if verbose:
            ...
            print("Configuration:")
            lr_desc = "(scheduled)" if self.config.use_lr_schedule else "(fixed)"
            print(f"  • Learning rate: {self.config.learning_rate} {lr_desc}")
        ...
        for step in range(self.config.max_iterations):
            try:
                ...
                if verbose and (step % 20 == 0 or grad_norm < self.config.tolerance):
                    print(
                        f"{step:3d}\t{params[0]:.6f}\t{params[1]:.6f}\t"
    ...
    def _simulate_detection(
        ...
        return experiment.run_sensing_protocol(
            rotation_angles=rotation_angles, with_photon=with_photon, protocol_type="theta1_theta2"
        )


def create_optimizer(
    optimizer_type: str = "adam", learning_rate: float = 0.1, max_iterations: int = 100, **kwargs
) -> Optimizer:
    """
