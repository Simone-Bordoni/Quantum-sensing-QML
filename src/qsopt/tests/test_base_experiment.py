...
class ConcreteExperiment(Experiment):
    ...
    def optimize_rotations(self, num_steps=100, batch_size=1, tolerance=1e-6,
                          verbose=True, verbose_step=10, callback=None, **kwargs):
        """Minimal implementation."""
        from qsopt.core.callback import OptimizationCallback
        return OptimizationCallback()
...
class TestExperimentAbstractClass:
    ...
    def test_default_optimize_measurement_times_not_implemented(
        ...
        with pytest.raises(NotImplementedError, match="does not implement measurement time optimization"):
    ...
    def test_default_save_experiment_report_not_implemented(
        ...
        with pytest.raises(NotImplementedError, match="does not implement experiment report saving"):
